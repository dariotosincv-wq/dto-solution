...
function Dashboard({ access, session, error }) {
  ...
  const shortcuts = [[Truck, 'Veicoli', COMPANY_ROUTES.vehicles, vehiclesAllowed], [FileText, 'Ispezioni', COMPANY_ROUTES.inspections, inspectionsAllowed], [ShieldCheck, 'Verifica PDF', COMPANY_ROUTES.verifyPdf, tools], [LayoutGrid, 'Confronta ispezioni', COMPANY_ROUTES.comparePdf, tools]]
  ...
  return <div className="company-page fleet-dashboard">
    ...
    <div className="fleet-primary fleet-primary--lower">
      <Panel title="Da risolvere prima delle partenze" icon={TriangleAlert} className="fleet-priorities"><p className="fleet-caption">Segnalazioni tecniche aperte. I danni fotografici restano nelle schede veicolo.</p>{summary.open === null ? <Empty>{unavailable('reports', vehiclesAllowed)}</Empty> : summary.open.length === 0 ? <div className="fleet-success"><CircleCheck size={36} /><strong>Nessun problema da verificare</strong><p>Non risultano segnalazioni tecniche aperte nei dati disponibili.</p></div> : <div className="fleet-report-list">{summary.open.map(report => { const vehicle = summary.byId.get(report.vehicle_id); return <article className="fleet-report" key={report.report_id}><span className="fleet-icon"><Truck size={25} aria-hidden="true" /></span><div><strong>{vehicle?.plate || 'Mezzo non presente nel catalogo'}</strong><small>{vehicle?.internal_code || '—'}</small></div><div><span className="fleet-badge">Aperta</span><strong>{VEHICLE_REPORT_LABELS[report.report_type] || report.report_type}</strong>{report.description && <p>{report.description}</p>}<small>{dateLabel(report.reported_at)}{report.driver ? ` · Driver: ${report.driver}` : ''}</small></div>{vehicle && <Link to={COMPANY_ROUTES.vehicle(vehicle.vehicle_id)}>Dettagli <ArrowRight size={16} /></Link>}</article> })}</div>}</Panel>
