  ...
  return <div className="company-page assignments-page">
    ...
    <section className="daily-catalog" aria-labelledby="daily-title"><header><h2 id="daily-title">Assegnazioni</h2><span>{displayed.length} di {data.drivers.length} driver</span></header><p className="daily-save-hint">Salvataggio automatico alla scelta del mezzo.</p>
      <table className="daily-table"><caption className="visually-hidden">Assegnazioni della data selezionata</caption><thead><tr><th scope="col">Driver</th><th scope="col">Codice</th><th scope="col">Mezzo assegnato nella data selezionata</th></tr></thead><tbody>
        {displayed.map(driver => { const current = data.assignments.find(item => item.driver_id === driver.driver_id); return <tr key={driver.driver_id}><th scope="row">{driver.last_name} {driver.first_name}</th><td data-label="Codice">{driver.driver_code || 'Senza codice'}</td><td className="daily-control"><select aria-label={`Mezzo di ${driver.first_name} ${driver.last_name}`} value={current?.vehicle_id || ''} disabled={saving === driver.driver_id} onChange={(e) => void assign(driver.driver_id, e.target.value)}><option value="">Non assegnato</option>{data.vehicles.map(vehicle => <option key={vehicle.vehicle_id} value={vehicle.vehicle_id} disabled={data.assignments.some(item => item.vehicle_id === vehicle.vehicle_id && item.driver_id !== driver.driver_id)}>{vehicle.internal_code} · {vehicle.plate}</option>)}</select><small className="assignment-feedback" aria-live="polite">{saving === driver.driver_id ? 'Salvataggio…' : saved === driver.driver_id ? 'Salvato' : ''}</small></td></tr> })}
      </tbody></table>
