...
export default function DriversPage() {
  ...
  return <div className="company-page driver-directory">
    ...
    <section className="drivers-catalog" aria-labelledby="drivers-title">
      ...
      <table className="drivers-table"><caption className="visually-hidden">Driver della tua organizzazione</caption><thead><tr><th scope="col">Cognome</th><th scope="col">Nome</th><th scope="col">Codice</th><th scope="col">Stato</th><th scope="col">Azioni</th></tr></thead><tbody>
        {displayed.map(driver => <tr key={driver.driver_id}><th scope="row">{driver.last_name}<span className="drivers-mobile-name"> {driver.first_name}</span></th><td className="drivers-first-name">{driver.first_name}</td><td data-label="Codice">{driver.driver_code || 'Senza codice'}</td><td data-label="Stato"><span className="drivers-badge" data-status={driver.status}>{driver.status === 'active' ? 'Attivo' : driver.status === 'archived' ? 'Archiviato' : driver.status}</span></td><td className="drivers-actions">{driver.status === 'active' && <button type="button" aria-label={`Archivia ${driver.last_name} ${driver.first_name}`} onClick={() => void archive(driver)}><Archive size={16} aria-hidden="true"/>Archivia</button>}</td></tr>)}
      </tbody></table>
