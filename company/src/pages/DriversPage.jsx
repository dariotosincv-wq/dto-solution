...
export default function DriversPage() {
  ...
  const refresh = useCallback(async () => { try { const value = await loadCompanyDrivers(session.access_token); setItems(value.items); const active = await Promise.all(value.items.filter(driver => driver.status === 'active').map(async driver => { const result = await loadDriverOperationalQr(session.access_token, driver.driver_id); return result.token ? driver.driver_id : null })); setActiveQrIds(new Set(active.filter(Boolean))) } catch { setError('Anagrafica driver non disponibile.') } }, [session.access_token])
  useEffect(() => { if (!canManageVehicles(access)) return undefined; const timer = window.setTimeout(() => { void refresh() }, 0); return () => window.clearTimeout(timer) }, [access, refresh])
