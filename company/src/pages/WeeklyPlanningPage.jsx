    ...
    try {
      ...
      if (input.action === 'COPY_PREVIOUS') {
        ...
        setData(current => {
          ...
          const replace = list => [...list.filter(item => entryKey(item.driver_id, item.assignment_date) !== entryKey(input.driver_id, input.assignment_date)), ...(result.item ? [result.item] : [])]
  ...
  return <div className="company-page weekly-planning">
    ...
    {!loading && <>
      ...
      <div className="planning-grid-wrap"><table className="planning-grid"><caption className="visually-hidden">Pianificazione dal {start} al {dates[6]}</caption><thead><tr><th scope="col">Driver ↑</th><th scope="col">Profilo</th><th scope="col">Pianificate</th><th scope="col">Scost.</th>{dates.map(date => <th scope="col" key={date}>{dateLabel(date, { weekday: 'short' })}<small>{dateLabel(date, { day: '2-digit', month: '2-digit' })}</small></th>)}<th scope="col">Azioni</th></tr></thead><tbody>{displayed.map(driver => {
        ...
        })}<td data-label="Azioni"><button className="planning-row-action" aria-label={`Completa settimana di ${driver.last_name} ${driver.first_name}`} disabled={busy || Boolean(error)} onClick={() => { const date = dates.find(day => !entries.has(entryKey(driver.driver_id, day))) ?? start; setSelection({ driver, date, planned: entries.get(entryKey(driver.driver_id, date)), effective: effective.get(entryKey(driver.driver_id, date)) }) }}>•••</button></td></tr>
