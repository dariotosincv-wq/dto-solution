...
export default function LoginPage() {
  ...
  if (session && access?.role !== 'UNION_GUEST') return <Navigate to={location.state?.from?.pathname || COMPANY_ROUTES.dashboard} replace />
  if (session && access?.role === 'UNION_GUEST') return <main className="login-page"><section className="login-card"><img src="/brand/dto-solution-horizontal.svg" alt="DTO Solution" /><p className="company-kicker">Area Aziende</p><h1>Accesso non autorizzato</h1><p className="notice notice--error" role="alert">Questo account non è autorizzato ad accedere all’Area Aziende. Utilizza l’accesso Area Enti.</p><button className="button-secondary" type="button" onClick={signOut}>Esci e usa un altro account</button></section></main>
