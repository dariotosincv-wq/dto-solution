...
function verifiedPage(page) {
  let lines = page.lines.map(clean).filter((line) => !/^(?:ANN? ?[ÙUV]|MA © N V|i gp i Iedl|L \| j D I A NL|LA \| plat|LZ EE|7 \+ A JU|[A-ZÀ-ÿ]{0,3}[ |/\\_—#©]{3,}|[nm]{1,2} pz E al)/i.test(line))
  ...
  if (page.page === 10) lines = lines.map((line) => line.replace(/condizioni d$/, 'condizioni di').replace(/^4a offerto/, 'offerto').replace(/^Pi$/, '').replace(/stabilizzazioni di\/$/, 'stabilizzazioni di').replace(/^N c\./, 'c.').replace(/^DX} suddetti/, 'suddetti').replace(/^SS maggiormente/, 'maggiormente').replace(/^È A questo/, 'A questo').replace(/^do intesa/, 'intesa').replace(/^di RI ricorso/, 'ricorso').replace(/^Ni essere/, 'essere').replace(/^s tipologia/, 'tipologia').replace(/^Ea distribuzione/, 'distribuzione').replace(/^NN aziendale/, 'aziendale').replace(/^PI dell'attività/, "dell'attività").replace(/^7 \* 50%/, '• 50%').replace(/^65%/, '• 65%').replace(/^XS personale/, 'personale'))
