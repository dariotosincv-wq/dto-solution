...
import { CalendarDays, FileText, Download, Settings, BookOpen, Scale, ShieldCheck, Check, Clock3, LifeBuoy, Laptop, CloudOff, Save, ArrowRight } from 'lucide-react'
...
export default function DriverAreaPage() {
  ...
  return <article className="driver-dashboard">
    ...
    <section className="driver-dashboard__rights" aria-labelledby="driver-rights-title"><div className="container">
      ...
      <div className="driver-dashboard__documents">{documents.map(({ icon: Icon, title, text, cta, to }) => <article className="driver-dashboard__document" key={to}><Icon size={27} strokeWidth={1.7} aria-hidden="true" /><h3>{t(...title)}</h3><p>{t(...text)}</p><Link to={to}>{t(...cta)}<ArrowRight size={18} aria-hidden="true" /></Link></article>)}</div>
    ...
    <div className="container">
      <section className="driver-dashboard__privacy" aria-labelledby="driver-privacy-title"><span className="driver-dashboard__shield"><ShieldCheck size={35} strokeWidth={1.6} aria-hidden="true" /></span><div><h2 id="driver-privacy-title">{t('I tuoi dati restano tuoi', 'Your data stays yours')}</h2><p>{t('Turni e buste paga vengono elaborati localmente nel browser. Nessun archivio cloud DTO Solution. Puoi creare un backup locale quando vuoi.', 'Shifts and payslips are processed locally in your browser. No DTO Solution cloud archive. Create a local backup whenever you want.')}</p><ul>{[[Laptop, 'Elaborazione locale', 'Local processing'], [CloudOff, 'Nessun cloud DTO', 'No DTO cloud'], [Save, 'Backup disponibile', 'Backup available']].map(([Icon, it, en]) => <li key={it}><Icon size={32} aria-hidden="true" />{t(it, en)}</li>)}</ul></div></section>
      <section className="driver-dashboard__help" aria-labelledby="driver-help-title"><LifeBuoy size={48} aria-hidden="true" /><div><h2 id="driver-help-title">{t('Hai bisogno di aiuto?', 'Need help?')}</h2><p>{t('Consulta le informazioni disponibili oppure contatta DTO Solution.', 'Browse the available information or contact DTO Solution.')}</p></div><details className="driver-dashboard__faq"><summary><BookOpen size={22} aria-hidden="true" />{t('Guida e FAQ', 'Guide and FAQ')}</summary><div><strong>{t('Dove sono salvati i dati?', 'Where is my data stored?')}</strong><p>{t('Turni e buste paga restano nel browser utilizzato. Usa Backup e ripristino per conservarne una copia locale.', 'Shifts and payslips stay in the browser you use. Use Backup and restore to keep a local copy.')}</p><strong>{t('Da dove cominciare?', 'Where should I start?')}</strong><p>{t('Configura il profilo contrattuale, poi apri Turni o Busta Paga. I documenti sono disponibili nella sezione Conosci i tuoi diritti.', 'Configure your contract profile, then open Shifts or Payroll. Documents are available under Know your rights.')}</p></div></details><Link className="driver-dashboard__contact" to="/contatti">{t('Contattaci', 'Contact us')}</Link></section>
    </div>
  </article>
}
