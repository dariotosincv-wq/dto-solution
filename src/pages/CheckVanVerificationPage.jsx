...
import { ShieldCheck, Search, Upload, FileCheck2, Fingerprint, ListChecks, Laptop, Mail, BookOpen } from 'lucide-react'
import './checkvan-verification-company.css'

const MAX_SIZE_MIB = MAX_CHECKVAN_PDF_BYTES / 1024 / 1024

function CheckVanVerificationPage({ companyDesign = false }) {
  const { t } = useI18n()
  ...
  return (
    <>
      ...
      <section className={`page-section checkvan-verification-page${companyDesign ? " verification-company" : ""}`}>
        <div className="container checkvan-verification-layout">
          {companyDesign ? <header className="verify-hero"><div><p className="eyebrow">STRUMENTI CHECKVAN</p><h1>Verifica <span>PDF</span></h1><h2>Controlla l’autenticità dei tuoi documenti CheckVan</h2><p>Seleziona fino a {MAX_CHECKVAN_BATCH_FILES} PDF generati da Driver Utility e verifica se corrispondono ai documenti registrati nel sistema.</p><div className="verify-benefits"><span><ShieldCheck/>File sul tuo dispositivo</span><span><Fingerprint/>Controllo integrità e originalità</span><span><ListChecks/>Riepilogo dei risultati</span></div></div><div className="verify-art" aria-hidden="true"><div className="verify-paper back">CheckVan<i/><i/><i/></div><div className="verify-paper">CheckVan<i/><i/><i/><i/><ShieldCheck/></div><Search className="verify-lens"/><p>Documenti sicuri,<br/>aziende più tranquille.</p></div></header> : <header className="checkvan-verification-hero">
          ...
          {!companyDesign && <aside className="checkvan-privacy-note" aria-label={t('Informazioni sulla privacy', 'Privacy information')}>
          ...
          <form className="checkvan-verification-form" onSubmit={handleSubmit}>
            <div className="checkvan-file-field">
              ...
              {companyDesign && <div className="verify-upload-intro"><Upload size={42}/><strong>Scegli i documenti da verificare</strong><span>I PDF restano sul tuo dispositivo.</span></div>}
              <input
                id="checkvan-pdf"
                ref={fileInputRef}
                type="file"
            ...
            {companyDesign && <details className="verify-how" id="verify-guide"><summary>Come funziona?</summary><p>Seleziona i PDF e avvia la verifica. Il browser calcola l’impronta SHA-256 di ogni file e invia soltanto questa impronta al registro per cercare una corrispondenza. Il risultato appare qui sotto. Nessun PDF viene caricato.</p></details>}
          </form>
          ...
          {companyDesign && <>
            <section className="verify-info"><article><ShieldCheck/><div><h2>Documento originale</h2><p>Verifica se il PDF corrisponde esattamente a un documento registrato.</p></div></article><article><Search/><div><h2>Controllo dell’hash</h2><p>Confronta l’impronta SHA-256 con il registro CheckVan.</p></div></article><article><FileCheck2/><div><h2>Esito della verifica</h2><p>Consulta il riepilogo e il risultato di ogni documento.</p></div></article></section>
            <section className="verify-privacy"><ShieldCheck/><div><h2>I tuoi dati restano tuoi</h2><p>Il PDF resta sul dispositivo. Il calcolo dell’hash è locale; soltanto l’impronta viene inviata al registro per la verifica.</p></div><span><Laptop/>Hash calcolato localmente</span><span><FileCheck2/>Nessun upload PDF</span></section>
            <footer className="verify-help"><div><h2>Hai bisogno di aiuto?</h2><p>Scopri come funziona la verifica o contattaci.</p></div><a href="#verify-guide" onClick={() => { const guide = document.getElementById('verify-guide'); if (guide) guide.open = true }}><BookOpen/>Guida alla verifica</a><a href="/contatti"><Mail/>Contattaci</a></footer>
          </>}
        </div>
      </section>
    </>
