...
function CheckVanComparisonPage() {
  ...
  return <>
    ...
    <section className="page-section checkvan-comparison-page"><div className="container comparison-layout">
      ...
      {!documents.length && comparisonMode === 'cloud' && <><aside className="checkvan-privacy-note comparison-cloud-privacy"><span className="checkvan-privacy-note__icon" aria-hidden="true">✓</span><div><strong>I PDF vengono recuperati in modo sicuro dall'archivio aziendale e utilizzati solo per il confronto.</strong></div></aside><CloudComparisonPicker initial={cloudItems} onReady={handleCloudReady} />{progress && <p className="checkvan-local-error" role="status">{progress}</p>}{error && <p className="checkvan-local-error" role="alert">{error}</p>}<style>{'.checkvan-comparison-page .checkvan-verification-form, .checkvan-comparison-page .checkvan-privacy-note:not(.comparison-cloud-privacy) { display: none; }'}</style></>}
