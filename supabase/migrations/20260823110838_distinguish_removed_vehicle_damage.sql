...
alter table public.checkvan_vehicle_damages
  drop constraint if exists checkvan_vehicle_damages_status_check,
  add constraint checkvan_vehicle_damages_status_check
    check (status in ('PENDING','CONFIRMED','REJECTED','REPAIRED','REMOVED')) not valid;
alter table public.checkvan_vehicle_damages
  validate constraint checkvan_vehicle_damages_status_check;

alter table public.checkvan_vehicle_damages
  drop constraint if exists checkvan_vehicle_damages_actor_check,
  add constraint checkvan_vehicle_damages_actor_check check (
    (status='PENDING' and reported_by_device_id is not null and confirmed_by_auth_subject is null)
    or status in ('CONFIRMED','REJECTED','REPAIRED','REMOVED')
  ) not valid;
alter table public.checkvan_vehicle_damages
  validate constraint checkvan_vehicle_damages_actor_check;

alter table public.checkvan_vehicle_damage_events
  drop constraint if exists checkvan_vehicle_damage_events_event_type_check,
  add constraint checkvan_vehicle_damage_events_event_type_check
    check (event_type in ('CREATED','UPDATED','CONFIRMED','REJECTED','REPAIRED','REMOVED','DRIVER_WITHDRAWN')) not valid;
alter table public.checkvan_vehicle_damage_events
  validate constraint checkvan_vehicle_damage_events_event_type_check;
...
begin
  ...
  elsif p_action='REMOVE' and old_d.status='CONFIRMED' then next_status:='REMOVED';event_name:='REMOVED';
  ...
  update public.checkvan_vehicle_damages set
    ...
    repaired_at=case when next_status='REPAIRED' then now() else repaired_at end,
