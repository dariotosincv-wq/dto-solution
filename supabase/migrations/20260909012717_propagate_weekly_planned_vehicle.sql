-- Fill only later empty planned shifts. Effective overrides and routes remain untouched.
...
begin
  ...
  if action='SAVE' then
    ...
    if state='TURNO' and vehicle is not null then
      with updated as (
        update public.checkvan_planned_assignments
        set vehicle_id=vehicle,updated_by=p_auth_subject,updated_at=now()
    ...
    insert into public.checkvan_daily_requirements(organization_id,week_start,assignment_date,required_drivers,updated_by)
    select p_organization_id,start_date,assignment_date+7,required_drivers,p_auth_subject from public.checkvan_daily_requirements where organization_id=p_organization_id and week_start=start_date-7
