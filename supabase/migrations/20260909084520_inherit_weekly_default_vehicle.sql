-- A later TURNO inherits the driver's first planned vehicle in the same week.
-- This leaves explicit vehicles, routes and all effective overrides untouched.
...
declare
  ...
  action text := p_input->>'action'; state text := p_input->>'work_status';
  current_revision integer; copied integer := 0; copied_requirements integer := 0;
  changed jsonb; propagated jsonb := '[]'::jsonb;
begin
  ...
  if action='SAVE' then
    if state='TURNO' and vehicle is null then
      ...
      where a.organization_id=p_organization_id and a.week_start=start_date and a.driver_id=driver and a.work_status='TURNO' and a.vehicle_id is not null
      order by a.assignment_date asc limit 1;
    end if;
    ...
    if state='TURNO' and vehicle is not null then
      with updated as (
        update public.checkvan_planned_assignments set vehicle_id=vehicle,updated_by=p_auth_subject,updated_at=now()
        where organization_id=p_organization_id and week_start=start_date and driver_id=driver
          and assignment_date>day_date and work_status='TURNO' and vehicle_id is null
        returning *
    ...
    insert into public.checkvan_planned_assignments(organization_id,week_start,assignment_date,driver_id,work_status,vehicle_id,route,notes,updated_by)
    select p_organization_id,start_date,a.assignment_date+7,a.driver_id,a.work_status,a.vehicle_id,a.route,a.notes,p_auth_subject from public.checkvan_planned_assignments a join public.checkvan_drivers d on d.id=a.driver_id and d.organization_id=a.organization_id
    where a.organization_id=p_organization_id and a.week_start=start_date-7 and d.status='active' on conflict (organization_id,assignment_date,driver_id) do nothing;
    get diagnostics copied = row_count;
    insert into public.checkvan_daily_requirements(organization_id,week_start,assignment_date,required_drivers,updated_by)
    select p_organization_id,start_date,assignment_date+7,required_drivers,p_auth_subject from public.checkvan_daily_requirements where organization_id=p_organization_id and week_start=start_date-7 on conflict (organization_id,assignment_date) do nothing;
