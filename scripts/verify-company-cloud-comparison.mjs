...
const parserModule = "export const CHECKVAN_CATEGORIES=[{id:'front-full',it:'Anteriore',en:'Front'}];export const platesDiffer=()=>false;export const validatePdfFile=()=>null;export const releaseComparison=async()=>{};export const readCheckvanPdf=async(file,progress)=>{progress(1,1);return {metadata:{plate:'CLOUD1',vehicle:'S116',inspectionType:'Presa',date:'01/03/2030',time:'08:00'},photos:{},loadingTask:{destroy:async()=>{}}}}"

const browser = await chromium.launch({ channel: 'chrome', headless: true })
try {
  const context = await browser.newContext()
  let failure = ''; let delayed = false
  const requests = []
  ...
  await context.route('**/cloud-*.pdf', route => route.fulfill({ contentType: 'application/pdf', body: '%PDF-1.4\n%%EOF' }))
